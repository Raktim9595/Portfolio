...
function toggleSkills(e) {
  ...
  // }
  let link = e.target.closest(".skills__content");
  if(!link) return;
  let link__lists = link.querySelector(".skills__list");
  link__lists.classList.toggle("skills__list--open");
  link.querySelector(".skills__arrow").classList.toggle("skills__arrow--open");
}
...
tabs.forEach(tab => {
  tab.addEventListener("click", () => {
    ...
    tabs.forEach(tab => {
      tab.classList.remove("qualification__active--button");
    })
    tab.classList.add("qualification__active--button");
  });
});
